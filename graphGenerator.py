...
def dropEdges(G,numEdges,maxClique,cliqueNodes):
    vCount,eCount = getGraphStats(G)
    ...
    for v1 in G.keys():
        ...
        for v2 in G.keys():
            if G[v1][v2] == 0:
                eNoDrop[v1][v2] = 1
    for v1 in cliqueNodes:
        for v2 in cliqueNodes:
            eNoDrop[v1][v2] = 1
...
def genGraph2(numVert,numEdges,maxClique):
    ...
    vOrder = defaultdict(int)
    G = defaultdict(lambda: defaultdict(int))
    cliqueNodes = generateClique(G,vOrder,numVert,maxClique)
    genSubCliqeGraph(G,numVert,maxClique)
    dropEdges(G,numEdges,maxClique,cliqueNodes)
    return G
...
if __name__ == "__main__":
    ...
    G = genGraph2(numVert,numEdges,maxClique)
